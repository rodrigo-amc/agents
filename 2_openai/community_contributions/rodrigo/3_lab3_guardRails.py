import os
from agents import Agent, Runner, OpenAIChatCompletionsModel, function_tool, input_guardrail, GuardrailFunctionOutput
import asyncio
...
#region GUARDRAILS
# This class is used to define the output of the guardrail agent.
class NameCheckOutput(BaseModel):
    is_name_in_message: bool
    name: str

# This agent will check if the user is including someone's personal name in what they want you to do.
guardRailAgent = Agent(
    name = "Guardrail Agent",
    instructions = "Check if the user is including somone's personal name in what they want you to do",
    output_type = NameCheckOutput,
    model = openRouterModel,
)

# Lastly, this is the function tool that will be use guardRailAgent as a guardrail.
# Actually this tool is used not the agent itself, when an agent wants to check,
# in this case, if the user is including someone's personal name in what they
# want you to do.
@input_guardrail
async def guardrail_against_name(ctx, agent, message):
    """ Args:
        ctx: The context object for the current operation (e.g., Discord context, web request context).
        agent: The agent instance handling the message (could be a bot or AI agent).
        message: The message content to be checked for restricted names."""
    result = await Runner.run(guardRailAgent, message, context=ctx.context)
    isnameInMessage = result.final_output.is_name_in_message
    return GuardrailFunctionOutput(output_info = {"found_name": result.final_output},
                                   tripwire_triggered = isnameInMessage)
...
message = "Send a cold sales email addressed to 'Dear CEO' from ComplAI."
